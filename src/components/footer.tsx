...
export default function Footer() {
  ...
  return (
    <footer className="relative bg-black text-gray-400 py-16 px-4 sm:px-8 md:px-24 border-t border-white/5 overflow-hidden z-20">
      ...
      <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-amber-500 to-transparent animate-pulse" />

      {/* Decorative moving bg elements */}
      <div className="absolute -bottom-10 left-1/3 w-[300px] h-[300px] rounded-full bg-amber-900/5 blur-[90px] pointer-events-none" />
          ...
          {socialLinks.map((social) => {
            ...
            return (
              <a
                ...
                className="p-2 rounded-lg border border-white/5 hover:border-amber-500/20 bg-white/5 hover:bg-amber-500/10 text-gray-400 hover:text-white transition-all duration-300"
              >
      ...
      <div className="max-w-6xl mx-auto pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs">
        ...
        <motion.button
          whileHover={{ y: -3 }}
          onClick={scrollToTop}
          className="flex items-center gap-1.5 px-4 py-2 rounded-full border border-white/10 hover:border-amber-500/30 bg-white/5 hover:bg-amber-500/10 text-xs font-semibold text-gray-300 hover:text-white transition-all duration-300"
        >
          Back to Top
